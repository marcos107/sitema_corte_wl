    ...
    function lista_tarefas()
    {
        ...
        if (true) {
            ...
            $status_desenho = Ferramentas::array_index(
                Ferramentas::array_pesquisa($desenhos->find(), "id", Ferramentas::array_index($corte_data, ['id_desenho'])),
                ['status']
            );
            ...
            // >>>>>>> ATUALIZAÇÃO NA CONSULTA AOS DESENHOS <<<<<<
            // Filtra os desenhos do usuário com status "pendente" ou "cortando"
            $desenhos_data = $desenhos
            ...
            // O restante do código monta as listas para exibição,
            // lembrando que agora os registros já possuem status pendente ou cortando.
            if (count($corte_data) != 0) {
                foreach ($desenhos_data as $key => $value) {
                        ...
                        ) == $processo)
                    {
                        ...
                        if (Ferramentas::decodificador($value['status']) == "pendente" ||
                            Ferramentas::decodificador($value['status']) == "cortando") {
                                ...
                                if (Ferramentas::decodificador($value['status']) == "cortando") {
                                    ...
                                    <tr>
                                        ...
                                        <td>' . Ferramentas::decodificador(
                                            Ferramentas::array_index(
                                                Ferramentas::array_pesquisa($usuario_data, 'id', $value['desenhista']),
                                                ['nome']
                                            )
                                        ) . '</td>
                                        <td>' . Ferramentas::decodificador(Ferramentas::remove_id_file(Ferramentas::decodificador($value['nome']))) . '</td>
                                        <td>' . Ferramentas::decodificador(
                                            Ferramentas::array_index(
                                                Ferramentas::array_pesquisa($empresa_data, 'id', $value['empresa']),
                                                ['nome']
                                            )
                                        ) . '</td>
                                        <td>' . Ferramentas::decodificador(
                                            Ferramentas::array_index(
                                                Ferramentas::array_pesquisa($empreendimento_data, 'id', $value['empreendimento']),
                                                ['nome']
                                            )
                                        ) . '</td>
                                        <td>' . Ferramentas::decodificador(
                                            Ferramentas::array_index(
                                                Ferramentas::array_pesquisa($finalidade_data, 'id', $value['finalidade']),
                                                ['nome']
                                            )
                                        ) . '</td>
                                        <td>' . $tags . '</td>
