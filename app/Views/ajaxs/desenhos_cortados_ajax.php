...
`;
  ...
  function lista_recolcoar() {
    $.ajax({
      ...
      success: function(response) {
        ...
        modalTitulo.textContent = "Solicitações para recolocação";
  ...
  function processo_lista() {
    $.ajax({
      ...
      type: "POST",
      data: { contexto_tela: 'desenhos_cortados' },
      dataType: "json", // Indicar que o retorno é em formato JSON
...
<!-- DXF viewer: UMD + módulo ES que expõe window.DXFViewer -->
...
<script src="<?= base_url('public/assets/visualizar.js'); ?>"></script>
<script>
