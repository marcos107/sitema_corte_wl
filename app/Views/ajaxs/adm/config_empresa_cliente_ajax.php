<script>
  ...
  document.getElementById("empresa_cliente_novo").addEventListener("input", function () {
    var input = this;
    var maxLength = 17;
    ...
    $.ajax({
      ...
      success: function (response) {
        ...
        if (!response.desenho) {
          ...
          modal_titulo.textContent = "Modificar a empresa: " + response.nome;
          ...
          inputElement.id = 'empresa_cliente_novo';
          ...
          alert_personalizado('Modificar', 'Empresa já está em uso.');
        }
      }
    });
    ...
    $.ajax({
      ...
      success: function (response) {
        if (!response.ok) {
          ...
          alert_certo('Alteração', 'Empresa/cliente Modificado com sucesso.');
          lista();
        }
