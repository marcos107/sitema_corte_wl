...
<script>
  ...
  function renderizarAcaoPrincipalLista(row) {
    ...
    if (tipo === 'ind') {
      ...
      return '<div class="wl-row-actions"><button type="button" onclick="baixar(' + indice + ')" class="btn btn-sm btn-primary wl-row-action-main">Ver</button></div>';
    }
  ...
  function enviarArquivosProjetoSelecionados(projetoId, files) {
    ...
    function finalizarEnvio() {
      ...
      atualizarEstadoModalArquivosProjeto("loading");
      carregarArquivosProjeto(projetoId);
      lista(true);
...
function marcarArquivos(id) {
  ...
  function removerArquivoProjeto(id) {
    ...
    $.ajax({
      ...
      success: function(response) {
        ...
        var projetoId = response.projeto_id || window.wlModalProjetoState.projetoId || window.wlModalProjetoState.listaId;
        atualizarEstadoModalArquivosProjeto("loading");
        carregarArquivosProjeto(projetoId);
        lista(true);
      },
  ...
  function confirmar_ind(id, nome) {
    if (mostrarConfirmacao("Finalizar Projeto" + nome)) {
      $.ajax({
