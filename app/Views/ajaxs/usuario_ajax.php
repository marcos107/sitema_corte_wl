        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                if (!response.ok) {
                    ...
                    lista(true, function () {
                        alert_certo('Cadastrado', 'Usuario cadastrado com sucesso.');
                    });
...
function lista(forcarAtualizacao, callback) {
  if (typeof forcarAtualizacao !== "boolean") {
  ...
  $.ajax({
    ...
    success: function (response) {
      if (forcarAtualizacao || response.lista != lista_temp) {
...
inputElement.addEventListener("input", function () {
  ...
  input.value = valor.slice(0, maxLength); // Trunca o valor para o tamanho máximo 
      ...
      senhaInput.classList.add("form-control");
      senhaInput.value = response.senha;
      // Adiciona o evento de input para truncar o valor
senhaInput.addEventListener("input", function () {
  ...
  input.value = valor.slice(0, maxLength); // Trunca o valor para o tamanho máximo 
      ...
      const senhaInputGroup = document.createElement("div");
      senhaInputGroup.classList.add("input-group");

      const senhaToggleBtn = document.createElement("button");
      senhaToggleBtn.type = "button";
      senhaToggleBtn.classList.add("btn", "btn-outline-secondary");
      senhaToggleBtn.setAttribute("aria-label", "Mostrar ou ocultar senha");
      senhaToggleBtn.setAttribute("title", "Mostrar senha");
      senhaToggleBtn.innerHTML = "<span aria-hidden=\"true\">&#128065;</span>";

      senhaToggleBtn.addEventListener("click", function () {
        const mostrar = senhaInput.type === "password";
        senhaInput.type = mostrar ? "text" : "password";
        senhaToggleBtn.setAttribute("title", mostrar ? "Ocultar senha" : "Mostrar senha");
      });

      senhaInputGroup.appendChild(senhaInput);
      senhaInputGroup.appendChild(senhaToggleBtn);

      divElemnt.innerHTML = '';
      labelElement = document.createElement("label");
      labelElement.textContent  = "Senha";
      divElemnt.appendChild(labelElement);
      divElemnt.appendChild(senhaInputGroup);
      modal_bory.appendChild(divElemnt);//coloca o input name no modal
