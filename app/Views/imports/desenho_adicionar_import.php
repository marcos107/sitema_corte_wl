...
function desenho_modal_ind() {
    ...
    function getSelectWithGlobal(selectId, globalId) {
        var select = document.getElementById(selectId);
        if (select && select.value !== '') {
            return select;
        }

        var globalSelect = document.getElementById(globalId);
        return globalSelect && globalSelect.value !== '' ? globalSelect : select;
    }

    function getSelectValueWithGlobal(selectId, globalId) {
        var select = getSelectWithGlobal(selectId, globalId);
        return select ? select.value : '';
    }

    function getSelectTextWithGlobal(selectId, globalId) {
        var select = getSelectWithGlobal(selectId, globalId);
        if (!select || select.selectedIndex < 0 || select.value === '') {
            return '';
        }
                    ...
                    for (let j = 0; j < desenhos.length; j++) {
                        ...
                        for (let h = 1; h < 4; h++) {
                            ...
                            if (document.getElementById("tag" + h + "_novo_" + j)) {
                                ...
                                let empreendimentoNome = getSelectedOptionText("empreendimento_novo_" + j);
                                let finalidade = document.getElementById("finalidade_novo_" + j).value;

                                let subpastas = response.tags?.[empreendimento]?.[finalidade]
                                    || response.tags?.[empreendimentoNome]?.[finalidade]
                                    || [];
    ...
    function confirmarModal() {
        if (tipo_input == 'mult') {
            ...
            for (let j = 0; j < desenhos.length; j++) {
                if (desenhos[j] != null) {
                    data = {
                        empresa_id: getSelectValueWithGlobal("empresa_cliente_novo_" + j, "empresa_cliente_novo_todos"),
                        empresa: getSelectTextWithGlobal("empresa_cliente_novo_" + j, "empresa_cliente_novo_todos"),
                        empreendimento_id: getSelectValueWithGlobal("empreendimento_novo_" + j, "empreendimento_novo_todos"),
                        empreendimento: getSelectTextWithGlobal("empreendimento_novo_" + j, "empreendimento_novo_todos"),
                        finalidade: getSelectValueWithGlobal("finalidade_novo_" + j, "finalidade_novo_todos"),
                        prioridade: getSelectValueWithGlobal("prioridade_novo_" + j, "prioridade_novo_todos"),
                        tag1: getSelectValueWithGlobal("tag1_novo_" + j, "tag1_novo_todos"),
                        tag2: getSelectValueWithGlobal("tag2_novo_" + j, "tag2_novo_todos"),
                        tag3: getSelectValueWithGlobal("tag3_novo_" + j, "tag3_novo_todos"),
                        desenho: desenhos[j]
                ...
                if (document.getElementById("tag1_novo_0") != null && (response.toString() != lista_temp_c3 || efeturar)|| efeturar) {
                    ...
                    for (let h = 1; h < 4; h++) {
                        ...
                        funcao.onchange = function() {
                            ...
                            for (let j = 0; j < desenhos.length; j++) {
                                if (document.getElementById("tag" + h + "_novo_" + j)) {
                                    var selectElement_todos = document.getElementById("tag" + h + "_novo_todos");
                                    var selectElement = document.getElementById("tag" + h + "_novo_" + j);
                                    var possuiOpcao = Array.from(selectElement.options).some(function(option) {
                        ...
                        let empreendimentoNome = getSelectedOptionText("empreendimento_novo_todos");
    ...
    function selecionarValorSelectDependencia(id, valor) {
        ...
        for (var i = 0; i < select.options.length; i++) {
            ...
            var textoOpcao = select.options[i].textContent || '';
            if (
                valorOpcao === valor ||
                normalizarValorPreselecaoDependencia(valorOpcao) === valorNormalizado ||
                normalizarValorPreselecaoDependencia(textoOpcao) === valorNormalizado
            ) {
    ...
    function preSelecaoDependenciaCompleta(preselecoes) {
        ...
        for (var i = 0; i < checks.length; i++) {
            ...
            if (
                elemento &&
                valorEsperado &&
                normalizarValorPreselecaoDependencia(elemento.value) !== normalizarValorPreselecaoDependencia(valorEsperado) &&
                normalizarValorPreselecaoDependencia(getSelectedOptionText(checks[i][0])) !== normalizarValorPreselecaoDependencia(valorEsperado)
            ) {
                return false;
            }
        }
    ...
    function tentarAplicarPreselecaoDependencia(tentativa) {
        ...
        if (preselecoes.empresa && indiceEmpresaCabecalho > 0) {
            ...
            if (empreendimentoCabecalho && empreendimentoCabecalho.options.length <= 1 && !wlDependenciaBootstrapState.empreendimento) {
                ...
                value_empreendimento_c(empresaCabecalho.value, indiceEmpresaCabecalho, desenhos.length);
            }
        }
