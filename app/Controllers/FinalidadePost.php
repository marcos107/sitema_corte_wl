...
class FinalidadePost extends Ferramentas
{
      ...
      if (count($msg) == 0) {
        ...
        if (count(Ferramentas::array_pesquisa($desenhos_data, 'finalidade', $lista['id'])) == 0) {//verifica se esta em uso essa finalidade
          ...
          $alteracao->insertWithDetails(
            ...
            [
              [
                "valor_antes" => Ferramentas::array_index(Ferramentas::array_pesquisa($finalidade_data, 'id', $id), ['nome']),
                "valor_depois" => Ferramentas::norma_lizar_str($finalidade),
                "campo" => "nome"
              ]
            ]
          );
            ...
            $date = [
              'nome' => Ferramentas::norma_lizar_str($finalidade),
  ...
  function finalidade_lista()
  {
    if ($this->request->isAJAX()) {
      ...
      $lista = array();
      $lista_session = array();
      $tokens = $_SESSION['subpasta_finalidade_tokens'] ?? [];
      ...
      foreach ($finalidade_data as $key => $value) { //cria a lista
        ...
        if ($value['status'] == 'ativo') {
          $finalidadeId = (int) ($value['id'] ?? 0);
          $token = array_search($finalidadeId, $tokens, true);
          if ($token === false) {
            $token = bin2hex(random_bytes(16));
            $tokens[$token] = $finalidadeId;
          }

          $temp['id'] = $token;
          $temp['finalidade'] = Ferramentas::decodificador($value['nome']);
          $lista_session[$finalidadeId] = $temp['finalidade'];
          $lista[] = $temp;
        }
      ...
      $_SESSION['subpasta_finalidade_tokens'] = $tokens;
      //retorna a lista para o ajax
      $data = [
        "lista" => $lista
