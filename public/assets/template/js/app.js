...
(function () {
	...
	function findMenuLinkByCurrentPath(menuRoot, currentPath) {
		if (!menuRoot || !currentPath) {
			return null;
		}

		var directMatch = menuRoot.querySelector('[href="' + currentPath + '"], [href="/' + currentPath + '"]');
		if (directMatch) {
			return directMatch;
		}
		...
		for (var i = 0; i < links.length; i++) {
			...
			try {
				...
				var pathSegment = pathname.substring(pathname.lastIndexOf("/") + 1).toLowerCase();
				if (pathSegment === normalizedPath) {
					return links[i];
				}
			} catch (error) {
				continue;
	...
	function twoColumnMenuGenerate() {
		...
		if (defaultValues && (isTwoColumn == "twocolumn" || defaultValues["data-layout"] == "twocolumn")) {
			...
			if (currentPath) {
				// navbar-nav
				var a = findMenuLinkByCurrentPath(document.getElementById("navbar-nav"), currentPath);
	...
	function initTwoColumnActiveMenu() {
		...
		if (currentPath) {
			if(document.body.className == "twocolumn-panel"){
				var activeTwoColumnLink = findMenuLinkByCurrentPath(document.getElementById("two-column-menu"), currentPath);
				if (activeTwoColumnLink) {
					activeTwoColumnLink.classList.add("active");
				}
			}
			// navbar-nav
			var a = findMenuLinkByCurrentPath(document.getElementById("navbar-nav"), currentPath);
	...
	function initTopbarComponents() {
		...
		if (document.getElementsByClassName("notification-check")) {
			function emptyNotification(){
				var notificationItemsTabContent = document.getElementById("notificationItemsTabContent");
				if (!notificationItemsTabContent) {
					return;
				}
				...
				if (document.querySelectorAll(".notification-item").length > 0) {
					...
					var emptyNotificationElem = notificationItemsTabContent.querySelector(".empty-notification-elem")
					if(!emptyNotificationElem){
						notificationItemsTabContent.innerHTML += '<div class="empty-notification-elem text-center px-4">\
				...
				element.addEventListener("change", function (el) {
					...
					if (el.target.closest(".notification-item").classList.contains("active")) {
						if (notificationActions) {
							(checkedCount > 0) ? notificationActions.style.display = 'block': notificationActions.style.display = 'none';
						}
					} else {
						if (notificationActions) {
							(checkedCount > 0) ? notificationActions.style.display = 'block': notificationActions.style.display = 'none';
